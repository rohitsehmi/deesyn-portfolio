...
## Project overrides (16_Revolut)

This skill is general. Where it disagrees with the project, the project wins:

- **Shadows are not available in site chrome.** revolut.com has zero `box-shadow`; depth is colour-banding and luminance. This skill lists shadow/glow as part of the motion materials palette — that applies only to app mockups inside case studies. For the site itself, reach for blur, `clip-path`, opacity, transform, and band adjacency instead. See `tokens/README.md`.
- **Every value binds to a token.** Durations, easing curves, and any colour used in a transition are tokens, not literals. If a motion value needs to exist in more than one place, it belongs in `tokens/tokens.json` under `semantic.*`, not inlined. See `CLAUDE.md`.
- **Dual light/dark is not optional**, and a band owns the foreground of everything inside it. Any hover, active, or focus treatment must resolve correctly inside an `inverse` band without an override. See `docs/banding-system.md`.
...
| Hardcoded duration or easing literal | Bind to a token (project rule, see Project overrides) |
| Motion that only resolves in one theme | Verify inside an `inverse` band with zero overrides |
