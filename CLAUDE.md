...
**Neither has a Figma file, and their receipts fall back to Revolut's.** That is the one visible wart: the Figma receipt is the single per-host link, so `ticketmaster.deesyn.com` shows a URL with `Revolut` in it under a sentence about this brand's own file. Contained rather than solved, and the fix is the same one Wise and Healf had — a structural copy of the file, its cover node id read out rather than carried over, and a `brands.<brand>` block in `src/copy/how-this-was-built.json`.
