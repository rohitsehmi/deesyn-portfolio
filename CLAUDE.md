...
**`Content/Component Models` — added 2026-08-26, and it is the third figure drawn in elements rather than exported.** It fills the first of the three "Image needed" placeholders in `scaling-a-system`'s rejected-alternatives section: four design languages inside every component, drawn beside the base component with brand tokens on top that replaced it. Same mechanism as the other two — native controls plus `:has()`, no JavaScript, `role="img"` over the picture and never over the control, a `default*` prop purely so Chromatic can photograph the second state, and `alt` required because a picture built from elements has no missing image for anyone to notice.

- **The argument it makes is a COUNT, which is what decided its shape.** The exploration says every later change had to be correct in four places at once, so the control marks the row a change lands on and the reader finds it four times on the left and once on the right. A count is a thing to show rather than a thing to say, and the alt text has to carry the number for anyone who cannot see it.
- **The fidelity cost is drawn, and it has to be.** Exactly one decision moves out to the brand token chips, because the exploration admits in words that the pivot cost fidelity and a picture where the shipped model is all upside would be arguing with the paragraph beside it.
- **It marks a change to BEHAVIOUR, never to the look.** A change to the look genuinely lands in one place per brand in both models, so marking it would show two structures agreeing and prove nothing.
- **Every wash mixes with `transparent`, never with a band token**, and that is a correction rather than a preference. `color-mix(<token> n%, var(--semantic-band-sunken))` paints one specific band's fill; this ships on a `base` band and renders against all four in Storybook. `GovernanceTiers.css` still mixes against `band-sunken` in two places, which is a live inconsistency rather than a contrast failure — the measured pairs pass either way.
- **`Exploration.diagram` is a discriminated union now**, `{ kind: 'governance' | 'component-models' }`, rather than a field per diagram. At three figures a field each makes "exactly one artefact" a convention nothing enforces, which is precisely why `image` already wins over `carousel` wins over `diagram`.
- **No marks and no accents**, unlike the token-tiers figure next to it. That one's subject IS brand identity; this one's subject is where a decision lives, and three logos beside a fourth entry with no logo would read as an oversight rather than as the partner side genuinely not being a consumer brand.
...
**Thirteen components exist only in code**, with no Figma set: `Content/Section Heading`, `Prose`, `Metrics`, `Explorations`, `Hindsight`, `Contribution`, `CaseStudyTile`, `Parallax`, `Carousel`, `TokenTiers`, `GovernanceTiers`, `ComponentModels` and `ConvergenceScenarios`. Their contracts are things a variant cannot express, so building them in Figma would document them *less* precisely. `design/build-code-specs.mjs` measures them from source instead: props from the TypeScript declarations, tokens from their own stylesheets, don'ts from `usage-rules.json` like every other component. **25 React components, 25 with a published contract** — this file said six and 17/17 until 2026-08-10, when `CaseStudyTile` and `Parallax` had joined the list without it being written down, and said 20/20 until 2026-08-13.
...
**Public as of 2026-08-07**, alongside the repo. 69 stories across 25 story files, both themes.
