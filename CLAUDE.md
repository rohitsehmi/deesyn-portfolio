...
**`revolut.deesyn.com` was retired from the `vercel.json` allowlist at the same time** and now redirects to the apex like any other unmatched host.
...
**Both got their own Figma file on 2026-08-26, so all five brands now carry their own receipt.** Ticketmaster `nMD0jeuLCPuuKL3GIR0Da9`, Asos `uYcV5BQm3cQuaqQkOpcRM7`, both at cover node `56:4384` like Revolut and Healf, and both verified link-shared by an anonymous oEmbed 200 rather than from the share dialog.
...
**Unmatched hostnames redirect to the apex, added 2026-08-14.** `*.deesyn.com` is attached to the project, so before this every name someone guessed — `healf.deesyn.com`, `monzo.deesyn.com` — served the whole site. `vercel.json` now carries a redirect whose `missing.host` names the hosts that are real (`deesyn.com`, `www`, `wise`, `healf`, `ticketmaster`, `asos`) and sends everything else to `https://www.deesyn.com`. **`revolut` was on that list and came off on 2026-08-26**, so that subdomain now redirects to the apex like any other guessed name. Four things about it that are decisions rather than syntax:
...
**SETTLED 2026-08-26, for the apex.** An `×` lockup conventionally signals a partnership: sent directly to someone as a piece of work it is exactly right, and sitting on an open URL it implies an engagement that does not exist. Both halves of that were always true, and the mistake was treating it as one question. **The apex now carries the mark alone and each partner hostname keeps the lockup it was aimed at**, so the claim is only ever made to the reader it was made for. `noindex` (`public/robots.txt` and the `X-Robots-Tag` header in `vercel.json`) still contains the rest.
...
**Wise and Healf became real brands on 2026-08-19 rather than lockup swaps — see § Brand packs. Ticketmaster and Asos were added on 2026-08-26 and needed no new mechanism. Content is still what moves the needle.** 9 pages, 25 components, every check green, no gaps rendering on any live page, five brands live.
