...
**WHAT THE REAL FOUNDATIONS SAY ABOUT THE TOKEN-TIERS FIGURE, and it is not all good.** Checked on 2026-08-26 against the same source:

- **All three brand accents in `egds-token-tiers.ts` are wrong.** Expedia `#FCBA02` against a real `#FDDB32`, Hotels.com `#F6343F` against `#E61E43`, Vrbo `#054CC0` against `#006ED6` — 58, 27 and 41 apart in RGB distance. They were sampled off a PNG, which the data file says, and can now be sourced.
- **The button ramp is exact where it was sourced and a near-miss where it was inferred.** Rest `#1668E3` and active `#0E3672` match the real ramp exactly; hover `#124CA4` is 12 from the real `#0D4EAF` and disabled `#ABC9F5` is 5 from `#A6C9F7`. So the ×0.73 / ×0.52 ratios recorded for that figure were reverse-engineering something that is just the ramp's own 700 and 800 steps.
- **The one that changes the picture rather than a value: all three brands declare the SAME action ramp.** The figure gives each brand its own primary button — blue, red, navy. The foundations give Expedia, Hotels.com and Vrbo a byte-identical blue ramp and the same navy in the shared brand slots. Whether a brand's Primary button binds to that ramp is not settled by a foundations file alone, but Expedia's sampled button IS that ramp's 600 step exactly, which is the evidence there is. **Unresolved, and Rohit's to settle** — it is a claim about his own work, and correcting it would make the token-tiers figure say something sharper: identity differs, the action does not.
- **The six foundation greys it prints are all near-misses**, 5 to 20 off the real neutral ramp, and the semantic row's lavender has no counterpart in EGDS at all.
