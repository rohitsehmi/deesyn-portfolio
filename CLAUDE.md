...
**One build serves six brands, chosen by hostname.** `www.deesyn.com` shows the **Ro mark alone and names no company**; `revolut.deesyn.com` shows Ro × Revolut; `wise.deesyn.com` shows Ro × Wise (2026-08-13); `healf.deesyn.com` shows Ro × Healf (2026-08-14); `ticketmaster.deesyn.com` and `asos.deesyn.com` were added on 2026-08-26. Each partner carries its own hero copy and its own Figma receipt.
...
**REVOLUT WAS DEMOTED TO AN ORDINARY PARTNER, NOT REMOVED — and the first attempt got that wrong and shipped it.** `revolut` was deleted from `BRANDS` and from the `vercel.json` allowlist, so `revolut.deesyn.com` 307'd to the apex and the one hostname actually addressed to Revolut lost the lockup that was its entire reason for existing. **Making the default generic and retiring a partner are two different edits**, and only the first was asked for. Corrected the same day: `revolut` is a partner brand with its own CSS arm, its own hero line, its own Figma receipt and its own OG card, exactly like the other four.

**It renders the BASE tokens and must not be given a pack.** Every other partner resolves through `portfolio.json`; Revolut has no pack entry, so it falls back to the base collection — which is the Revolut reconstruction, and therefore right. Adding it to a pack would repaint the one brand the base palette was measured from.
...
**Unmatched hostnames redirect to the apex, added 2026-08-14.** `*.deesyn.com` is attached to the project, so before this every name someone guessed — `healf.deesyn.com`, `monzo.deesyn.com` — served the whole site. `vercel.json` now carries a redirect whose `missing.host` names the hosts that are real (`deesyn.com`, `www`, `revolut`, `wise`, `healf`, `ticketmaster`, `asos`) and sends everything else to `https://www.deesyn.com`. **`revolut` came off that list on 2026-08-26 and went straight back on the same day** — see § Brands: demoting it from default and retiring it are different edits, and the first attempt did the second by accident. Four things about it that are decisions rather than syntax:
...
**Revolut's own hostname shows two case studies, not three**, because it inherited what the default brand was offered when it held that slot and `scaling-a-system` still lists the other four. That is a restore rather than a decision — worth a look, since every other partner hostname shows all three.
