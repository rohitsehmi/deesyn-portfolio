...
**One build serves five brands, chosen by hostname.** `www.deesyn.com` shows Ro × Revolut; `wise.deesyn.com` shows Ro × Wise (2026-08-13); `healf.deesyn.com` shows Ro × Healf (2026-08-14); `ticketmaster.deesyn.com` and `asos.deesyn.com` were added on 2026-08-26. Each carries its own hero copy; each of the first three carries its own Figma receipt.
...
**ONE PACK CAN SERVE SEVERAL BRANDS.** `tokens/brands/portfolio.json` carries `brands: ['wise', 'healf', 'ticketmaster', 'asos', 'deesyn']`, because all four live brands were given identical ramps and four byte-identical pack files would be exactly the hand-maintained duplication every check in `design/` exists to stop. **Adding two brands to that list moved the pack checksum not at all**, which is the design working: the checksum hashes resolved *values*, and a brand name is not one. Split it the day they diverge: copy the file and narrow each list. `brand: 'x'` singular is still accepted.
...
**Settle the `×` lockup.** An `×` lockup conventionally reads as a partnership, which would imply an engagement that does not exist. Shared directly with a person it is fine; on an open URL it is not. The same question governs making the Storybook public, and it is now asked five times — Ro × Revolut, Ro × Wise, Ro × Healf, Ro × Ticketmaster and Ro × Asos, on five hostnames. **Each new hostname raises the stakes on it rather than spreading them**, because an `×` lockup that appears once reads as a study and one that appears five times reads as a claimed client list.
