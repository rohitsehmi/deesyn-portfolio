...
| Default brand | **`deesyn`, naming no company** | 2026-08-26. The apex carries the Ro mark alone; the `×` lockup is per partner hostname. The palette and icons stay Revolut-derived and unattributed. |
...
**One build serves six brands, chosen by hostname.** `www.deesyn.com` shows the **Ro mark alone and names no company**; `wise.deesyn.com` shows Ro × Wise (2026-08-13); `healf.deesyn.com` shows Ro × Healf (2026-08-14); `ticketmaster.deesyn.com` and `asos.deesyn.com` were added on 2026-08-26; `spotify.deesyn.com` on 2026-09-01. Each partner carries its own hero copy and its own Figma receipt.

**THE DEFAULT BRAND WAS `revolut` UNTIL 2026-08-26 AND IS NOW `deesyn`, which settles the `×` question rather than dropping it.** The apex is the one address somebody reaches by typing it, with no covering message, and it was the single host rendering another company's logotype in its chrome and their name in its first sentence. An `×` lockup conventionally reads as a partnership: exactly right on a subdomain sent to a named person, and an implied engagement on an open URL. **The answer is to scope the lockup, not to delete it** — it now appears only where it was aimed, and the residue is a mark and a sentence that belong to nobody but Rohit.

**What that touched, and what it deliberately did not.** `src/data/brands.ts`, the `×` gating in `Logo.tsx`, one CSS arm, the hero line, the receipts list, the OG card and two `vercel.json` rules. **The palette and the icon set were left exactly as they are** — the tokens still reconstruct Revolut's public brand values and the 15 icons are still their assets verbatim, because nothing on the page attributes either and both are already unnamed. That is a scope decision taken deliberately and it is the honest limit of the change: the site no longer *names* Revolut on the apex, and it is still *built* from a study of them. Removing that would be a different project — 249 tokens off their checksum and an icon set redrawn.

**REVOLUT WAS RETIRED ON 2026-09-01. They turned the work down, so the one hostname addressed to them has nobody left to address.** `revolut` is out of `BRANDS`, out of the `vercel.json` allowlist, out of `PARTNER_WORDMARKS`, out of both copy files and out of the Storybook toolbar, and `public/og-revolut.png` is deleted. `revolut.deesyn.com` now 307s to the apex like any other unrecognised name, which is the wildcard redirect doing exactly what it was built for.

**That is the SAME removal an earlier attempt made by accident on 2026-08-26, and the distinction still holds.** That attempt was demoting the default and took the hostname down as a side effect; it was reverted the same day because **making the default generic and retiring a partner are two different edits** and only the first had been asked for. This one is the second edit, asked for on its own terms. The git history reads as a flip-flop and is not: `7f1a2c5` changed the apex, `220bbdf` put the partner back, and this restores the same end state for the opposite reason. Worth knowing before anyone reverts it as a repeat of the earlier mistake.

**What it deliberately does NOT touch is the study underneath.** The 249 tokens still reconstruct Revolut's public brand values from their live CSS, the 15 icons are still their assets verbatim, `docs/revolut-design-foundations.md` still records where every value came from, and the shared Figma file is still titled **"Revolut"**. None of that is attributed on any page and none of it was ever named there. **Retiring the hostname stops the site ADDRESSING them; it does not stop the site being BUILT from a study of them**, and that remains the same deliberate scope decision recorded above rather than an oversight. Undoing it is a different project — 249 tokens off their checksum and an icon set redrawn.

**BRINGING A RETIRED BRAND BACK IS ONE REVERT, AND THERE IS DELIBERATELY NO "HIDDEN" STATE TO PUT ONE IN.** The question was asked directly and it is worth answering here, because the architecture makes the answer counter-intuitive and a future session will ask it again.

**There is no dormant setting because a dormant brand would not be hidden.** This site is isolation by ROUTING, not by build: one prerendered document answers every hostname and CSS hides whatever does not apply. So a brand left in `BRANDS` but dropped from the `vercel.json` allowlist is still in the HTML of every page every OTHER client's hostname serves. Measured on the built `index.html` rather than reasoned about — it carries all four hero sentences at once, "for Wise.", "for Healf.", "for Ticketmaster.", "for ASOS.", plus all four logotypes. **A retired partner in that state is a company that declined the work, still named in the markup handed to every other client**, readable in view-source and readable by anything fetching the page without running the stylesheet — which is increasingly how this URL travels. That is worse than removal, not gentler than it.

**So removal IS the hidden state, and git is where it is hidden.** The brand is genuinely absent from anything anyone can fetch, and fully intact one command away. `verify-vercel-config.mjs` enforces that the two halves move together: a brand in `BRANDS` whose host does not match fails the build, and so does a host matching with no brand behind it. **A half-restore cannot ship**, which is the property that makes the revert safe to do quickly.
...
**What a revert will NOT do is re-decide the question.** The apex has named no company since 2026-08-26 and restoring a partner does not change that; these are the two different edits recorded above, and they stayed different in both directions.

**The base palette IS the Revolut reconstruction, and retiring the brand does not change that.** Every partner resolves through `portfolio.json`; `revolut` never had a pack entry, so it fell back to the base collection, which was measured from them. **Nothing had to move when it was retired** — the base collection is what the apex renders and what every check is calibrated against, and it is unchanged. If the reconstruction is ever unpicked, that is a token job and not a brand job.

**The fourth and fifth brands cost nothing structural, and that is the result worth recording.** Healf, the third, broke three things at once because the mechanism had only ever been exercised by two. Ticketmaster and Asos needed no new mechanism at all: `src/data/brands.ts`, one CSS arm each, one name each in the inline script, one hero string, one lockup path, one line in the pack's `brands` list, two `vercel.json` rules. **The two things they did surface were both latent bugs of the same shape as Healf's — a code path that had never seen a second case.** `Logo.tsx` dropped `evenOdd` on the partner logotype, which was invisible while Wise and Healf were both non-zero and would have filled every counter in Asos solid; and `how-this-was-built.astro` looked up `brands[brand].links` unconditionally, so a brand with no Figma file of its own failed the build on `undefined.links` rather than falling back. Adding the *n*th thing is how you find out what only worked for *n-1*.

**Both got their own Figma file on 2026-08-26, so every partner brand carries its own receipt.** (The apex carries none, deliberately — see above.) Ticketmaster `nMD0jeuLCPuuKL3GIR0Da9`, Asos `uYcV5BQm3cQuaqQkOpcRM7`, both at cover node `56:4384` like Revolut and Healf, and both verified link-shared by an anonymous oEmbed 200 rather than from the share dialog.

**A DUPLICATED FIGMA FILE KEEPS THREE LINKS BACK TO THE FILE IT CAME FROM, and only one of them is visible.** Both new files were duplicates, and both carried the identical set: the cover's `Brand/Logo` instance was bound to a **remote** component — another file's 192-wide lockup, not the local 233-wide one — the cover frame's fill was bound to a **remote** `bg/canvas`, and the frame's explicit variable mode was set on a **remote collection id**, so the file's own `02 Semantic` never received a mode at all.

**Those three cancelled each other out, which is why the covers looked fine.** Every part resolved through the *other* file's Dark mode, giving white-on-navy. Re-pointing only the logotype broke the symmetry and the cover rendered near-black navy on pure black — the local half reading Light while the background fell back to the literal `#000000` its dead binding still carried. **Editing the Marks page alone would have left each cover showing another brand's lockup**, which is exactly what "make sure it lands on cover too" was asking about.

The fix is per file: swap the instance to the local component, rebind the frame fill to the local `bg/canvas`, set the local collection's Dark mode, and clear the stale remote mode entry. Verified by sweeping every node — **3,875 nodes in each file, zero remote modes and zero remote instances left**. Two remote bindings survive in each, `Element/Hotels` and `Element/Expedia` on the case-study imagery, and those are correct: they are depicted brand colours in artwork, not system tokens.

**Centre a cover on the ARTWORK, not on the instance box.** Ticketmaster's logotype fills the 233-wide slot exactly, so centring its box was right by luck. Asos stops at 195.03 and centring the box would have sat the mark 83px left of centre on a 1920 cover. Both are now measured — equal left and right margins — rather than assumed.
...
**`deesyn` is the default and carries no attribute**, so www, the apex, any unrecognised subdomain and a client running no JavaScript all get it. It is also the only brand the inline script will not set from `?brand=`, which is not an oversight: every default arm in `base.css` keys off the attribute being ABSENT, so setting it for the default would hide the default's own content. Verified by loading `?brand=deesyn`, `?brand=revolut` and `?brand=nonsense` and confirming all three leave the attribute unset and render the mark.
...
**A whole case study can belong to one brand, as of 2026-08-14** — `brand?: Brand` on a `Study`, absent meaning every brand. `wise-placeholder` was the first user and was **deleted on 2026-08-17**; `scaling-a-system` was the second, scoped to the partner hostnames so the apex kept two studies, one per discipline. **THAT SCOPING WAS LIFTED ON 2026-09-01 and the field is now absent, so every brand sees all three** — Rohit's call, overruling the one-per-discipline argument. **Nothing in this list stopped being true**; what changed is that no live study currently uses the mechanism, which is worth knowing before anyone deletes it as unused. Five things it settled that a hidden `<span>` never tested:
...
**THE DEFAULT BRAND HAS NO FIGMA RECEIPT AT ALL, since 2026-08-26, and that is a deliberate hole.** Every partner block links its own file; the apex list is the one a stranger sees, and the only file the shared system actually lives in is titled **"Revolut"** in Figma. A URL slug can be written to say anything, but Figma normalises it to the real file name, so the reader would land on another company's name on the one hostname just cleared of it. **Two receipts that are true beat three where one leaks.**

**Restoring it is one JSON object**: rename or duplicate `UnybX8G5sQIEhLLZN2YFl6` to something company-neutral, then paste an item of a partner's shape at the head of `links.items` in `src/copy/how-this-was-built.json`. **The column count follows the list length** — `--hb-cols` is emitted per group from `links.items.length` — so nothing else has to change. That mechanism was added for this: `repeat(3, 1fr)` would have left the apex's two receipts in a three-track grid with a hole, which is the same bug the index grid had when a brand-specific study made the tile count differ, and the same lesson — **`display: none` removes a grid item and cannot remove a track.**

**Each brand has its own Figma file, and each is a structural copy of the Revolut one.** Revolut `UnybX8G5sQIEhLLZN2YFl6`; Wise `uFjZtr6cIu6JsR3sLpOsYP`; Healf `F0AUIJ19FASx5sUJUTfzGp`; Ticketmaster `nMD0jeuLCPuuKL3GIR0Da9`; Asos `uYcV5BQm3cQuaqQkOpcRM7`; **Spotify `5jW5pmAv9ATVJhCaJwLojO`, added 2026-09-01**, cover `56:4384` like the rest. **They stopped being lockup swaps on 2026-08-19** — Wise and Healf now carry their own palette in both the file and the repo, verified equal by checksum. See § Brand packs.

**That claim was half true, and the half that was false lasted twelve days.** The PALETTE was genuinely in the Healf file and still reproduces `2791783775` from inside it. **The LOCKUP never was**: Healf's `Brand/Logo` wordmark carried REVOLUT's logotype on its Marks page from the day the file was made until 2026-08-26. Nothing caught it because nothing looks — the site renders the lockup from `src/components/logo-paths.ts`, which had Healf's real path all along, so the live hostname was always correct and only the design file was wrong. **A checksum that covers colours says nothing about geometry**, and this is the gap: `brand-export.snippet.js` hashes variables, and no check reads a vector.

**All three duplicated files pointed at the SAME remote lockup, `80f2ff704b3e...` at 192x48, and it is in none of them.** So the common ancestor is a fourth document — almost certainly the superseded Healf file `oC393wMf4jcRjm8qbkWTeG`, which now 404s anonymously. Every brand file made by duplication inherits a link to a document that may already be gone, and the only symptom is that the cover keeps rendering correctly until you touch it.

**Fixed in all FOUR on 2026-08-26 and verified the same way each time:** local lockup, local `bg/canvas`, local Dark mode, artwork-centred cover, then a full sweep — **0 remote modes and 0 remote instances** in every file, and the palette checksum re-run from inside each one. All four reproduce `2791783775` across 107 entries, so Figma and the repo agree about colour by measurement in every brand.

**EVERY ONE OF THE FOUR CARRIED REVOLUT'S LOGOTYPE ON ITS MARKS PAGE.** Not one of them had ever had its own. The live sites were always right because they render from `logo-paths.ts`; only the design files were wrong, for as long as each has existed.

**Wise was the worst and is the reason to check rather than assume.** On top of the same three remote links it had **82 remote bindings on its Marks page**, against 2 in each of the others, resolving through the ancestor's collection — so the page documented Wise's system in **Revolut's palette**: `fg/primary` rendering `#191c1f` instead of `#0e1728`, `fg/secondary` `#717173` instead of `#6b7080`, `bg/subtle` `#f7f7f7` instead of `#f5f6f8`. The local palette was correct the whole time and simply was not what the page was bound to. 43 bindings were re-pointed by name to the local collection; the only remote ones left are `Element/Hotels` and `Element/Expedia`, which are artwork.

**Wise also points at a DIFFERENT ancestor from the other three** — `8f1e3fd8…` at 224x48 and collection `541aa753…`, where Ticketmaster, Asos and Healf all share `80f2ff70…` at 192x48 and `21f4208f…`. So there are at least two dead parent documents in this lineage, not one.

**WISE HAD TWO `Brand/Logo` MASTERS AND THEY HAD DIVERGED.** `19:4158` has **no page parent at all** — an orphan, alive only because instances still reference it — and **all 18 instances use it**: the cover, Nav, Footer, and every Template. The set actually shown on the Marks page, inside section `01 · Brand/Logo`, is a different one, `2001:115`, which nothing instantiates. So fixing the logotype made every rendered surface correct while the documented specimen still showed Healf, which is what Rohit spotted. **The other three files have exactly one set, on the page, used by all 18 instances** — Wise is the only one that split, and it is the reason to enumerate masters rather than take the first match.

**`2001:115` was also still 192 wide**, sized for Healf's artwork, and Wise runs to 219.51 — so it was resized to the shared 233 with the vector pinned to `MIN` for the duration and restored to `SCALE` after, which is the same procedure the ellipse bug taught.

**The prose was corrected in all four on 2026-08-26, in three places each**, because a file naming another company is wrong rather than merely stale: the on-canvas Marks text, the set `description`, and the **`spec/contract` plugin data**, which is the machine-readable copy and was the one still embedding "the Ro x Revolut lockup" after the canvas already read correctly. Names and the two false sizes (192x48, 224x48 → 233x48, the component's measured size) were substituted; nothing else was rewritten.

**The orphan was resolved on 2026-08-26 and the file now has one master.** Only **7 of the 18** instances were top-level; the other 11 carry an `I…;…` id, which means they are sub-instances mirroring their parent component's contents and cannot be swapped directly — they followed automatically the moment the 7 were re-pointed. Geometry was captured and re-applied per instance, so the cover stayed at 1024.23x211 and every nav lockup at 155.33x32. **`remove()` is a no-op on a parentless node**, so the retained copy still answers `getNodeByIdAsync`; what changed is that nothing references it and it is unreachable by walking the document, which is the outcome that matters. Check `removed`, `parent` and reachability rather than trusting that `remove()` did anything.

**THE REVOLUT FILE WAS SWEPT AND IS CLEAN** — one `Brand/Logo` on the Marks page carrying Revolut's own logotype, all 18 instances on it, **0 remote modes and 0 remote instances**, and only the two artwork bindings. `Deprecated/Brand Logo` is present as designed. So the base file was never the problem; every fault was introduced by duplication.

**Revolut's cover is genuinely `#000000`, and that coincidence is why this hid for so long.** Its frame binds to its own local `bg/canvas`, which really does resolve to black in Dark mode. In every duplicated file the black was a DEAD BINDING falling back to a stored literal. Identical on screen, opposite in cause — a broken brand cover was indistinguishable from the correct base one, which is exactly the failure mode this repo keeps meeting: **the symptom was not an error but a page that looked right.**

**Nothing in `verify:all` could have caught any of this and nothing should try.** No check reads a Figma file — the export snippets need plugin-only APIs and CI has no session — so lockup geometry, master duplication and stale prose are all caught by a person opening the file, or not at all. That is the same standing hazard `design/asset-provenance.json` exists for, one layer further out.

**SPOTIFY'S FILE CARRIED HEALF'S LOGOTYPE, NOT REVOLUT'S — 2026-09-01, and it corrects the claim above.** Every earlier duplicate inherited Revolut's; this one was duplicated from a file that had already been fixed, so it arrived showing **Ro x Healf** with the vector at Healf's exact 191.18 width. **The fault survives duplication whatever the parent** — what changes is only whose logotype you find. Check by reading path index 4 of the `Brand/Logo` wordmark's vector rather than by assuming which company it names.

**Its fault profile was otherwise BETTER than the others and worse in one place.** One `Brand/Logo` set, on its page, no orphan; and the Cover was already wired to the local component, the local `bg/canvas` and a local collection mode — none of the three remote links that cost a session in August. **What is entirely remote is the Templates page**: 523 remote bindings, 68 remote instances and 3 remote modes, all of them there. That is the same cross-file condition already recorded for the other brand files and it was left alone, because it is one shared problem across five documents rather than something Spotify introduced.

**Fixed by writing the repo's path in, not by drawing.** `vectorPaths` index 4 replaced with the committed Spotify path; the vector went 191.18 to 198.79 on its own, so **no resize was needed and the SCALE-constraint trap never came up**. The cover was then re-centred on the ARTWORK — 523.07 margins each side, against the 539.8 that had been right for Healf's narrower mark. Prose substituted in all three places the record names: three canvas text nodes, the set `description`, and the `spec/contract` plugin data. Verified by sweeping every text node in the file for the old name: zero left. Palette re-checked from inside the file, **107 entries reproducing `2791783775`**, with `L|fg/accent|#16625b` and `D|band/base|#0e1728` spot-checked.
...
`PARTNER_WORDMARKS` in `src/components/logo-paths.ts` keys the partner logotype by brand. Revolut is deliberately not in it — its logotype is the last entry of `LOCKUP_PATHS`, which is what a client running no JavaScript renders. All of them sit in the same `0 0 233 48` viewBox and share one slot starting at x=84.6, where the `×` glyph ends, fitted to y 8..40. Each logotype is a different width inside that box — Wise runs to 219.51, Asos to 195.03, Healf to 191.18 — and none is rescaled to match; the trailing space simply differs, which at nav sizes is invisible. One `<svg>` per brand would be the alternative and would duplicate the 16kB disc on every page.

**Ticketmaster is the first logotype that is WIDTH-bound rather than height-bound, and it looks smaller than the others on purpose.** Its wordmark is 7.49 wide for every 1 tall, so filling the slot's 32 of height would need 240 of width against the 148.4 there is. It sits 19.82 tall, centred in y 8..40, with visibly shorter ascenders than Healf's. The alternatives are scaling to a common x-height, which overflows the viewBox and clips, or widening the box for one brand, which costs the shared `<svg>` that makes this one path per brand. **A long, low wordmark reads long and low** — that is the artwork, not a defect in the fit.

**Ticketmaster's and Asos's paths came from each company's published SVG rather than from Figma**, and were then written INTO their Figma files from the repo rather than drawn there — the opposite direction from Wise and Healf.

**Figma's `vectorPaths` accepts only `M L C Q Z`, and refuses an arc outright** with "Invalid command at A". Both web-sourced logotypes carried `A` and `S`, so both were converted — `S` by reflecting the previous control point, `A` by splitting at 90 degrees or finer and fitting a cubic. That is an approximation, so it was proven rather than assumed: rendered before and after at 8x and diffed pixel by pixel, **14 of 715,776 differ for Ticketmaster and 5 for Asos**, every one an anti-aliased edge. The repo was moved onto the converted form so both sides describe one set of curves. **Wise and Healf were already arc-free because they came OUT of Figma**, which is the same constraint seen from the other side.

**The winding rule does not come with the path and must be set explicitly.** Ticketmaster is `NONZERO` and Asos is `EVENODD`; Asos is a single path whose counters are cut out of it, so carrying the wrong rule over fills every a, s, o and s solid. Nothing was traced: the committed path is the supplied data under a uniform scale and translate, the same rule `build-service-marks.mjs` and `build-brand-marks.mjs` already apply to somebody else's mark. The bbox was measured with a real browser's `getBBox()` and the emitted path re-measured afterwards — both land inside the slot and hold their source aspect to within 0.03%. The source, the scale and the translate are recorded in `logo-paths.ts` so the fit is reproducible; **do not nudge coordinates by hand, which is how a logo stops being the logo.**

**`PARTNER_WORDMARKS` is keyed by `PartnerBrand`, added 2026-08-26**, which is `Exclude<Brand, typeof DEFAULT_BRAND>` — so adding a brand without drawing its logotype is a typecheck failure rather than an undefined path that renders as a lockup with nothing after the `×`. That needed `DEFAULT_BRAND` to be `'revolut' as const satisfies Brand` rather than `: Brand`: annotating the constant widens it back to the whole union, `Exclude` comes out `never`, and the map silently accepts anything. One word, and it is the difference between a type and a check.

**Unmatched hostnames redirect to the apex, added 2026-08-14.** `*.deesyn.com` is attached to the project, so before this every name someone guessed — `healf.deesyn.com`, `monzo.deesyn.com` — served the whole site. `vercel.json` now carries a redirect whose `missing.host` names the hosts that are real (`deesyn.com`, `www`, `wise`, `healf`, `ticketmaster`, `asos`) and sends everything else to `https://www.deesyn.com`. **`revolut` came off that list on 2026-08-26 by accident, went straight back on the same day, and came off deliberately on 2026-09-01** — see § Brands. Removing it from this regex is the line that actually takes the hostname down; everything else about retiring a brand is cosmetic by comparison, and **nothing in `verify:all` checks that this list matches `BRANDS`.** Four things about it that are decisions rather than syntax:

**`design/verify-vercel-config.mjs` checks that allowlist against `BRANDS`, added 2026-09-01**, and it was added the day Revolut was retired because that is the day the list acquired a second way to be wrong. It asks the pattern about each hostname rather than parsing it — a regex is not a list, and re-deriving the alternation would be a second implementation of Vercel's matcher, which is the drift the check exists to stop. It fails in **both** directions, and both were verified by breaking the file and watching it refuse rather than by reading the code: a brand in `BRANDS` whose host does not match (the new hostname 307s and the lockup written for a named person is never seen), and a host that matches while its brand is gone (**a company that declined the work is still being addressed by name at an address they were already given**). It also asserts the apex and `*.vercel.app` still match, since losing the latter bounces every preview deployment to production.
...
**What is still Revolut-derived on every host:** the tokens and the icon set. Both are unnamed on the page and neither is attributed, and leaving them is a deliberate scope decision taken on 2026-08-26 rather than an omission — see § Brands. The **lockup** came off this list the same day and is now per-hostname, with the apex carrying none. The social card came off it on 2026-08-19 — see § Social and structured data, including the day it spent recorded as fixed while serving one brand's card to all three.
...
**ONE PACK CAN SERVE SEVERAL BRANDS.** `tokens/brands/portfolio.json` carries `brands: ['wise', 'healf', 'ticketmaster', 'asos']`, because all four partner brands were given identical ramps and four byte-identical pack files would be exactly the hand-maintained duplication every check in `design/` exists to stop. **Adding two brands to that list moved the pack checksum not at all, and removing one moved it not at all either**, which is the design working: the checksum hashes resolved *values*, and a brand name is not one. Split it the day they diverge: copy the file and narrow each list. `brand: 'x'` singular is still accepted.

**`deesyn` WAS IN THAT LIST AND CAME OUT ON 2026-08-26, and it was not merely inert while it sat there.** It was kept as a working reference on the reasoning that `verify-brands.mjs` prints an unserved brand rather than failing — the same argument as printing deprecated components. That held exactly as long as nothing served the name. The moment `deesyn` became the default brand it acquired a pack arm emitting `:root[data-brand='deesyn']`, which can never match because the default carries no attribute. **And it was worse than a dead rule**: `design/build-og-image.mjs` resolves a card's colours by asking which pack claims the brand, so the apex card was generated in the portfolio ramp (`#0e1728`) while the apex site rendered the base one (`#191c1f`). Two surfaces of one brand disagreeing about its own colour, caught by reading the build's own output rather than by any check. **A spare name in a list that something later starts serving is a live wire**, not a comment.
...
**Resolved 2026-08-04, and re-scoped 2026-08-26.** The chrome used to present Revolut's wordmark alone, which read as claiming their identity; the lockup fixed that by reading as work made *for* them. What the lockup could not fix is that the apex had no business making the claim to a stranger at all, so the default brand now renders the **mark alone** and the lockup is a per-partner-hostname thing. Nav and Footer still ask for `variant="wordmark"` on every host: the default brand's `×` and every logotype are gated away by `data-brand-only`, and `Logo.css` narrows the viewport to the disc. **One element, one copy of the 6.5kB disc, five outcomes.**
...
- **The brand marks are each company's own favicon, fetched by `design/build-brand-marks.mjs` into `src/data/egds-brand-marks.ts`.** They were a letter in a coloured disc first, which was a deliberate second-best: `design/build-service-marks.mjs` already holds the rule that a hand-traced logo is a wrong logo, and Simple Icons carries Expedia and Hotels.com as CC0 path data but not Vrbo, so a faithful set was unavailable and a mixed one would have read as an oversight. A favicon is the mark each company publishes at its own root for exactly this purpose, so all three come from one place and none is traced. **An `.ico` is a directory of frames and they are not all one format** — Expedia serves raw DIBs and the others embedded PNG, so both paths are decoded, and Expedia publishes nothing larger than 48px. `accent` survives the change and is separate from the button colour: Expedia's identity is yellow and its primary action is blue, which is the whole reason a feature layer exists.
...
**`Content/Design Languages` — added 2026-08-26, and it is the first figure on this site whose depicted values are SOURCED rather than sampled.** It fills the first "Image needed" placeholder in `scaling-a-system`'s rejected-alternatives section: what four design languages actually amounted to. Three brands, each showing the few values it declares on its own, over one shared block of four ten-step ramps they all declare identically. The control copies that shared block into every brand, which is the abandoned plan drawn.

- **Read from the live Figma file `Foundations`, `ftO20wB1T5EAIOulXsHeDQ`**, with `get_variable_defs` against the three brand instances on `0:1` — Expedia `1:3100`, Vrbo `1:5969`, Hotels `1:7206`. The token-tiers figure beside it is pixel-sampled from a PNG and its caption says "values are illustrative"; this one's says the opposite, and is entitled to.
- **`Unspecified / ` IS THE MACHINE-READABLE PART, and it is what makes the counts real.** In that response a variable prefixed `Unspecified / ` is one the brand does not declare, falling back to the neutral of the same tint — the foundation template says so in its own caption. So declared-versus-inherited is readable per brand, and **84 shared / 10 differing / 83 partial across a union of 177** is computed rather than asserted. `shared + differs` deliberately does not equal the union: 83 are declared by one or two brands and inherited by the rest, which is neither, and folding them into either is the flattering version.
- **No EGDS token names are rendered**, which is the decision already taken and not an oversight: a reader outside Expedia learns nothing from `color__accent__1__600`, and an internal namespace out of a private file is the disclosure this study cuts everywhere else. Plain-English labels for the same slots. The VALUES are a different matter — a brand's identity colour is on its home page.
- **The type face is NAMED, never set.** Reckless XPD, Recoleta and Lardent Pro Slab are licensed and absent here, so an "Aa" specimen would show three faces EGDS does not ship. Same rule as a hand-traced logo being a wrong logo.
- **Third time on this page that `fg/secondary` has been fine in one theme and under AA in the other** — 4.35:1 on the shared block's 8% wash in light, 9.58:1 in dark. Raised to `fg/primary`, and raised in BOTH places rather than only on the wash: the control exists so a reader can compare the block with the columns, and a label that changes colour when it is duplicated is a difference the picture is not drawing.
- **Two layout faults the browser found and the markup hid.** A two-column grid with the label forced to column two pushes the face onto a line of its own, which is how "Lardent Pro Slab" came out stacked three words deep. And a note living inside an `auto` column sizes that column to itself and squeezes the label beside it, which broke "Display face" across two lines on the one brand that has a note. The note is its own row now.
- **`measure-diagram-contrast.mjs` covers BRAND PALETTES as well as themes now**, which it did not until the day two brands were added. Every wash in these figures mixes a semantic token toward transparent, so the pair depends on the brand as well as the theme, and the set it measures is derived — the default plus the first brand of each pack — so splitting a pack grows it without anyone remembering to. 0 below AA across four figures, every palette, both themes, every state.
- **It replaced `Content/Component Models`, which lived for one commit (`1c477fd`).** That figure argued about where a decision lives, with four invented row labels; this one argues the same thing with the real numbers behind it. The old one is in git if the structural framing is ever wanted back.

**THE TOKEN-TIERS FIGURE WAS CORRECTED AGAINST THE SAME SOURCE, 2026-08-26**, because with the design-languages figure two sections above it the page had begun to contradict itself: the same three brands, twice, in different colours. Four faults, all now closed.

- **Its three brand accents were 27 to 58 apart in RGB distance from the real ones.** Sourced now: `#FDDB32`, `#E61E43`, `#006ED6`.
- **Its six foundation greys were 5 to 20 off** the real neutral ramp, and its semantic row was lavender and violet — colours that exist nowhere in EGDS. Both are real steps of the shared ramps now, and the hexes are printed, because a printed hex is a claim and this one can be checked.
- **THE LOAD-BEARING SENTENCE WAS FALSE.** "Foundation and semantic identical. Only the feature layer resolves per brand." The foundation tier is exactly where the brands differ — five brand slots and two accents. What is identical is the neutral, action, success and error ramps. The note now says so.
- **The three buttons were blue, red and navy, and there is no such difference.** All three brands declare the action ramp byte-identical, there is one Button with one binding, and Expedia's real button is that ramp's 600 step exactly. **All three buttons are the same blue now**, and the figure's point is better for it: pick a brand, the identity changes and the action does not.

**So the tier that sits empty until a brand is picked MOVED FROM LAST TO FIRST.** It was on feature, saying a brand differentiates at component level; it is on foundation, saying a brand sets its own block of raw values. **The copy was never wrong about this** — it says "a brand differentiates itself by overriding tokens rather than rewriting components" and never names a tier. Only the figure's note did.

**The ratios recorded for that button ramp were a wild goose chase and are gone.** "Hover ×0.73, active ×0.52, disabled 36% over white" described a button deriving its states arithmetically from its rest colour. Against the variables it does nothing of the kind: rest and active are the ramp's 600 and 800 steps exactly, hover and disabled 12 and 5 away from its 700 and 200 — the distance you get reading colours off a render. There is no ratio, there is a ramp, and the button uses four of its steps.

**A caution about the Figma Make exports.** The four `Foundation Template — EGDS` code exports handed over on 2026-08-26 disagree with the live file: the export carrying Vrbo's identity carries Expedia's partner ramp, and the explorer built beside them sets Reckless XPD as the display face for all three brands when only Expedia uses it. **Build from the file, not from the export** — the same rule as measuring the live site rather than trusting a secondhand spec.

**`Content/Convergence Scenarios` — added 2026-08-26, and it is the last of the three placeholders.** `scaling-a-system`'s rejected-alternatives section shipped with three `Media` placeholders whose alt strings literally opened "Placeholder for…", which a screen reader reads aloud; **the page now has none.** Five convergence scenarios on one axis, least ambitious first, with the one the leads recommended and the one the business took both marked.

- **THE THREE MIDDLE RUNGS CARRY NO LABEL, AND THAT IS THE COMPONENT'S WHOLE ARGUMENT.** The record names the two ends of the range and says the recommendation was "one of the middle options"; it does not name the middle three, and inventing them would be fabricating the contents of a real artefact — the same hazard as the four `nca-*` reconstructions. An unlabelled rung here is the shape of what is known, not a gap.
- **The recommendation is a BRACKET, never a mark on one rung.** "One of the middle options" is genuinely vague in the record, and a bracket draws that vagueness precisely where a dot would be a claim about which.
- **It has NO CONTROL, and that is the deliberate difference from the other two.** They hide a rejected alternative that cannot be drawn beside the shipped one, so a control is what lets a reader see both. Here the recommendation and the choice are on the scale at once and nothing is hidden, so a toggle would be a device — and three of them in one section is a tic a reader starts noticing instead of reading.
- **It was built vertically first and the measure is what settled it.** Five rungs stacked left occupy about a third of the article column with two labels in them, which reads as a figure with something missing rather than as a short one. A spectrum wants the width.
- **`1fr` was the bug, `minmax(0, 1fr)` is the fix.** A bare `1fr` is `minmax(auto, 1fr)`, so a column can never be narrower than its content — and the two end captions are `max-content`, so columns one and five inflated, the three between them shrank, and **the five nodes stopped being evenly spaced on a scale whose only job is even spacing.** The captions then met in the middle and overprinted each other.
- **The list items are `display: contents` and nothing carries a `grid-column`.** Each rung's four children fall into four rows of the same column by auto-placement, in source order. Each needs an explicit `grid-row` or the whole figure scrambles rather than degrading — it looked like a broken grid and it was a missing one. The empty note slot on four of the five rungs is load-bearing for the same reason.
- **`--cs-count` is hoisted from the component** because a node sits at a column centre, so the outermost two are `50% / count` in from the edges and CSS cannot derive that. Without it the axis labels align to the grid while the scale they describe is inset.
- **A copy wobble the diagram surfaced, and it is Rohit's to settle.** The exploration says the recommended middle option "kept the four systems independent on a shared token foundation", which is close to word for word how the process paragraph describes the LEAST ambitious end of the range. Both can be true, but as written they read as the same thing in two places on the scale. The title says "middle" outright, so the figure is built that way; if the recommendation was really the low end, the fix is one line in `src/data/egds-convergence-scenarios.ts`.
...
**SETTLED 2026-08-26, for the apex.** An `×` lockup conventionally signals a partnership: sent directly to someone as a piece of work it is exactly right, and sitting on an open URL it implies an engagement that does not exist. Both halves of that were always true, and the mistake was treating it as one question. **The apex now carries the mark alone and each partner hostname keeps the lockup it was aimed at — Revolut's included**, so the claim is only ever made to the reader it was made for. The lockup did not go anywhere; it stopped being what an unrecognised host renders. `noindex` (`public/robots.txt` and the `X-Robots-Tag` header in `vercel.json`) still contains the rest.

**What stays open is the Storybook**, which is one public URL carrying Revolut's real icon assets and, in `Marks/Logo`, every partner logotype at once. Scoping by hostname is not available there — it is a single Chromatic permalink — so the answer that worked for the site does not transfer.
...
**Thirteen components exist only in code**, with no Figma set: `Content/Section Heading`, `Prose`, `Metrics`, `Explorations`, `Hindsight`, `Contribution`, `CaseStudyTile`, `Parallax`, `Carousel`, `TokenTiers`, `GovernanceTiers`, `ConvergenceScenarios` and `DesignLanguages`. Their contracts are things a variant cannot express, so building them in Figma would document them *less* precisely. `design/build-code-specs.mjs` measures them from source instead: props from the TypeScript declarations, tokens from their own stylesheets, don'ts from `usage-rules.json` like every other component. **25 React components, 25 with a published contract** — this file said six and 17/17 until 2026-08-10, when `CaseStudyTile` and `Parallax` had joined the list without it being written down, and said 20/20 until 2026-08-13.
...
**Public as of 2026-08-07**, alongside the repo. 68 stories across 25 story files, both themes.

**A brand toolbar sits beside the theme one, added 2026-08-18**, and it closes the gap that `Marks/Logo` showed the Ro × Revolut lockup only — the one component whose whole job is to differ per brand was the one the workshop could not show differing. Revolut, Wise, Healf, Ticketmaster and Asos, with Revolut the default.
...
**Wise and Healf became real brands on 2026-08-19 rather than lockup swaps — see § Brand packs. Ticketmaster and Asos were added on 2026-08-26 and needed no new mechanism. Content is still what moves the needle.** 9 pages, 25 components, every check green, no gaps rendering on any live page, six brands live — the generic apex plus five partner hostnames.
...
1. **Copy across all six brands.** The last content job, and the only one that changes what a reader thinks. The per-brand override mechanism exists for anything naming a company — write it through the markdown round trip, not a field at a time. **The default brand's copy is the one to read first now**: since 2026-08-26 it names nobody, so it has to stand up to a stranger who arrived at the apex with no covering message. The hero is done (`{count} case studies, in full.`, now resolving to **Three** since the apex gained `scaling-a-system` on 2026-09-01); the rest of the pages have not been re-read with that reader in mind. **The apex now carries two design-system studies under the same tag**, which was the stated reason for holding it to two, so its index copy is the first thing to re-read.
...
3. **`og.png` unfurls as the DEFAULT card on every host. STILL OPEN, and it has now been recorded as closed twice on the same day. It stopped being a cross-brand leak on 2026-08-26 without being fixed**, which is worth separating from being fixed: the default card was Revolut's lockup, so a link pasted from the Wise hostname unfurled with another company's mark on it; the default card is now the bare Ro disc, so the same broken path shows a company-neutral mark instead of the wrong company's. Still the wrong card, and no longer an embarrassing one. The 2026-08-18 rewrite never ran because a real `dist/og.png` shadowed it; the 2026-08-19 rewrite runs and is never reached, because `og:image` is absolute and names www in every host's HTML. **Both closures verified the layer below the one that matters.** Contained by `robots.txt` and `X-Robots-Tag`, printed on every run by `verify-vercel-config.mjs`. **The brand-pack split landed on 2026-08-19 and did NOT fix it**, which is worth stating because this line predicted it would: a pack repaints a brand, it does not give a brand its own build, and only a per-brand build sets its own `site`. Twice now the open-items list has been right while the prose above it claimed otherwise.
...
~~**Settle the `×` lockup.**~~ **Settled 2026-08-26 for the site, and the shape of the answer is the part worth keeping.** An `×` lockup reads as a partnership: fine shared directly with a person, not fine on an open URL. Asked five times over five hostnames it stopped reading as a study and started reading as a claimed client list. **The fix was to scope it rather than to delete it** — the apex, which is the only address a stranger reaches by typing, carries the mark alone; each partner hostname keeps the lockup it was aimed at, and a reader only ever sees the one that was addressed to them.

**STILL OPEN FOR THE STORYBOOK, and scoping does not transfer there.** It is one public Chromatic permalink carrying Revolut's real icon assets and, in `Marks/Logo`, every partner logotype at once — there is no hostname to key off. The options are to leave it, to gate it, or to drop the partner logotypes from that one story, and none has been chosen.
