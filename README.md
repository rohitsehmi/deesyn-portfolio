...
Underneath the case studies there is a design system, comprising 249 tokens exported from Figma, 25 components, and a published contract for every one of them. It answers none of the questions above directly, and no reader asks about tokens or visual regression, so it is deliberately kept out of the way. It exists because it buys consistent, polished visuals cheaply, and because a system that verifies itself is a reasonable thing for a design engineer to be able to show.

**On the branding.** The design foundations here reconstruct a public brand's values from its live CSS, as a study in how a system of that kind is put together. This is independent work, and it was not commissioned by, affiliated with, or endorsed by any company whose name or marks appear in it. The default hostname names no company at all: it carries the author's own mark alone, and the `x` lockup appears only on the sibling hostnames each one was aimed at. That is deliberate, because an `x` lockup conventionally reads as a partnership, which is the right thing to say to a named recipient and the wrong thing to leave standing on an open address. What the siblings demonstrate is the multi-brand pipeline described below, not an engagement.
